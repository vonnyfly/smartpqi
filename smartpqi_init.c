	...
	struct pqi_queue_group *queue_group)
{
	...
	if (device->raid_level == SA_RAID_1) {
		...
		BUG_ON(offload_to_mirror >= layout_map_count);
	...
	struct pqi_queue_group *queue_group)
{
	...
	while (1) {
		...
		BUG_ON(request_id >= ctrl_info->max_io_slots);

		io_request = &ctrl_info->io_request_pool[request_id];
		BUG_ON(atomic_read(&io_request->refcount) == 0);
...
static inline void pqi_add_to_raid_bypass_retry_list(struct pqi_ctrl_info *ctrl_info,
